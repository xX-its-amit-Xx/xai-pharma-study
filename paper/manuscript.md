...
### 3.4 H3 (supported): explainers barely agree on which features matter
Median pairwise rank agreement is **0.05** (Spearman), far below any usable threshold. The disagreement is worse on high-dimensional ECFP (median 0.03) than on descriptors (0.12), but even on interpretable descriptors SHAP and LIME agree at only ρ = 0.09 (IG-vs-LIME 0.15). Toxicity endpoints are *not* more self-consistent than the rest (Mann–Whitney p = 0.24). Switching attribution library would substantially change the "important features" reported for a given prediction.
...
- LIME used a modest sampling budget (300 samples) for compute reasons; this may inflate LIME's measured instability and depress agreement. A robustness re-run at 1,000 samples is planned (P7) and the agreement claims will be re-checked.
- Faithfulness was measured primarily by mask-and-repredict comprehensiveness; a remove-and-retrain (ROAR) cross-check on a subset is planned.
...
- Single split realization per endpoint; multi-seed split repeats are a planned robustness check.
